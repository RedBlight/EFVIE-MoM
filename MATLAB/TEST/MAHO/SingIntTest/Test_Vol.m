...
% 
% 
% valA = SingIntVol( vr, v1, v2, v3, v4 );
% valN = AdaptiveSingIntVol( vr, v1, v2, v3, v4, 0, 0 );
% 
% ErrRel( valA, valN )
...
for idr = 1 : 4
	for idp = 1 : 24
		vecA = SingIntVol( v( :, idr ), v( :, vp( idp, 1 ) ), v( :, vp( idp, 2 ) ), v( :, vp( idp, 3 ) ), v( :, vp( idp, 4 ) ) );
		vecN = AdaptiveSingIntVol( v( :, idr ), v1, v2, v3, v4, 0, 0 );
...
for ide = 1 : 6
	...
	for idr = 1 : N
		...
		vr = vm + vecL .* f( idr );
		vecA = SingIntVol( vr, v1, v2, v3, v4 );
		vecN = AdaptiveSingIntVol( vr, v1, v2, v3, v4, 0, 0 );
		err = ErrRel( vecA, vecN );
		errMax = max( err, errMax );
		errMean = errMean + err;
		errMin = min( err, errMin );
